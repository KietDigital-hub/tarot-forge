...
d:\vibe code app\
├── lib/
│   ├── main.dart                                        # Điểm khởi động, ProviderScope, định tuyến ban đầu
│   ├── core/
│   │   ├── constants/
...
│   ├── features/
│   │   ├── customer_profile/                            # BƯỚC 1: Hồ sơ khách hàng
...
│   │   │   └── presentation/screens/customer_profile_screen.dart # Màn hình nhập thông tin, scale animation, nút hào quang
...
│   │   ├── deck_manager/                                # BƯỚC 3: Quản lý trọn bộ 78 lá bài [MỚI]
...
│   ├── deck_manager_test.dart                           # [PHASE 2] 11 tests trọn bộ 78 lá & DeckNotifier
│   ├── card_back_test.dart                              # [PHASE 2] 5 tests model mặt sau & CustomPainter
│   ├── batch_pdf_export_test.dart                       # [PHASE 2] 2 tests xuất PDF hàng loạt & in 2 mặt Duplex
│   └── deck_manager_flow_widget_test.dart               # [PHASE 2] 2 tests UI luồng duyệt bộ bài
└── PLAN.md                                              # Lộ trình tổng thể dự án từ Phase 1 đến Phase 4
```

---

## 3. CÁC TÍNH NĂNG ĐÃ HOÀN THIỆN (PHASE 1 + PHASE 2 + CẢI TIẾN NGHỆ THUẬT & MÀU SẮC)
1. **Luồng Hồ sơ Khách hàng (Customer Profile)**:
   - Tên khách hàng, Chủ đề, Ghi chú tâm niệm.
   - 20 Phong cách nghệ thuật & 12 Bảng màu phong thủy.
   - **Kho hình minh họa đa dạng (16 tranh lá bài riêng biệt)**:
     - 11 tranh mẫu mới chất lượng cao trong `assets/images/`: `gothic_dark.jpg` (XIII - CÁI CHẾT), `art_nouveau.jpg` (VI - ĐÔI TÌNH NHÂN), `witchcraft_folk.jpg` (IX - KẺ ẨN DẬT), `oriental_mystic.jpg` (X - BÁNH XE SỐ PHẬN), `egyptian_ancient.jpg` (V - ĐẠI TƯ TẾ), `cosmic_galaxy.jpg` (XXI - THẾ GIỚI), `cyberpunk_mystic.jpg` (VII - CỖ XE), `baroque_grand.jpg` (XI - CÔNG LÝ), `ink_wash_minimal.jpg` (0 - KẺ KHỜ), `tropical_forest.jpg` (VIII - SỨC MẠNH), `ocean_mystic.jpg` (II - NỮ TU TỐI CAO), cùng 5 lá bài gốc (`the_star.jpg`, `the_magician.jpg`, `the_moon.jpg`, `the_empress.jpg`, `the_emperor.jpg`).
     - Từng phong cách trong 20 phong cách nghệ thuật đều map chính xác đến tranh đặc trưng riêng biệt, không còn hiện tượng lặp lại 4-5 ảnh cũ.
   - **Hệ thống Dynamic Theme Palette (`TarotColorPalette`)**:
     - Cung cấp 12 bảng màu chủ đạo với các màu `darkAccent`, `lightAccent`, `primaryColor`, `secondaryColor`, và `getBackgroundGradient(isDark)`.
     - Độ tương phản đạt chuẩn WCAG, chữ và biểu tượng sắc nét trên cả chế độ Sáng và Tối.
     - Khi chọn tông màu ở Bước 1, toàn bộ giao diện `CardDesignerScreen` (Gradient nền màn hình, badge khách hàng, viền shimmering ánh kim, thanh công cụ, icon và các nút bấm chính) đổi màu đồng bộ theo đúng tông màu đã chọn.
2. **Quản Lý Trọn Bộ 78 Lá Bài (Deck Manager)**:
...
3. **Màn Hình Thiết Kế Lá Bài (Card Designer & 3D Flip)**:
...
4. **Thiết Kế Mặt Sau Lá Bài (Card Back Designer)**:
...
5. **Xuất File In Ấn Công Nghiệp Hàng Loạt (Batch Prepress Engine)**:
...
- **`flutter test`**: **42/42 tests PASSED** (toàn bộ unit & widget tests đều xanh).
