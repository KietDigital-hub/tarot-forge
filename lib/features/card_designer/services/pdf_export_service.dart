...
class PdfExportService {
  ...
  /// Generates print-ready PDF bytes for the given [TarotCard].
  ///
  ...
  static Future<Uint8List> generatePrintReadyPdf({
        ...
        build: (context) {
          return pw.Stack(
            children: [
              ...
              // Printer Crop Marks (Crosshairs pointing exactly to the 70x120mm trim corners)
              if (includeCropMarks)
                _buildCropMarks(
              ...
              // Prepress Color Calibration Bars at bottom slug
              pw.Positioned(
                bottom: 2 * mm,
                left: slugMarginPt,
                right: slugMarginPt,
                child: pw.Row(
                  ...
                  children: [
                    _buildColorPatch(PdfColors.cyan),
                    _buildColorPatch(const PdfColor.fromInt(0xFFE91E63)), // Magenta
                    _buildColorPatch(PdfColors.yellow),
                    _buildColorPatch(PdfColors.black),
                    _buildColorPatch(const PdfColor.fromInt(0xFFD4AF37)),
                    _buildColorPatch(const PdfColor.fromInt(0xFF1B0F2E)),
                  ],
                ),
              ),
            ],
    ...
    // --- PAGE 2: Exact 76 x 126 mm Full-Bleed Cut Page ---
    doc.addPage(
      pw.Page(
        pageFormat: const PdfPageFormat(
          fullWidthPt,
  ...
  /// Builds the 76x126mm bleed canvas content with the card elements.
    ...
    return pw.Container(
      ...
      child: pw.Stack(
        children: [
          ...
          // Footer: Card Title Banner & Subtitle
          pw.Positioned(
            bottom: bleedPt + 14,
            left: bleedPt + 16,
            right: bleedPt + 16,
  ...
  /// Renders standard offset printer crop marks at the 4 trim corners.
  static pw.Widget _buildCropMarks({
    ...
    // Exact Trim Box offsets on the sheet
    ...
    const markGap = 1.0; // 1pt gap from trim edge
